...
input_directory="/home/shilc/benchmarks/{$solver}"
output_directory="/home/shilc/SMT-test/results"
path_directory="/home/shilc/SMT-test/{$solver}_path"
...
mkdir -p "$output_directory/$solver"
rm -rf "$output_directory/$solver"/*
# 遍历所有组文件，并使用run.sh脚本处理每一组
for group_file in "${path_directory}/group_"*; do
    group_name=$(basename "$group_file")
    nohup ./run.sh "$group_file" "$input_directory" "$solver" "$time_t" > "${output_directory}/${solver}/${group_name}.log" 2>&1 &
done
