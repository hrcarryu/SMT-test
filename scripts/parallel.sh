...
input_directory="/home/shilc/benchmarks/${benchmarks}"
output_directory="/home/shilc/SMT-test/results"
path_directory="/home/shilc/SMT-test/paths/${benchmarks}_path"
...
mkdir -p "$output_directory/$solver/$benchmarks"
rm -rf "$output_directory/$solver/$benchmarks"/*
# 遍历所有组文件，并使用run.sh脚本处理每一组
for group_file in "${path_directory}/group_"*; do
    group_name=$(basename "$group_file")
    nohup ./scripts/run.sh "$group_file" "$input_directory" "$solver" "$time_t" > "${output_directory}/${solver}/${benchmarks}/${group_name}.log" 2>&1 &
done