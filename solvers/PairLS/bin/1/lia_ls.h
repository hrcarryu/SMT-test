...
struct lit_info{
    ...
    __int128_t lit_delt;
    int is_true;
    bool operator == (const lit_info &e){
        return (this->lit_idx == e.lit_idx);
        //&& (this->coff_lit == e.coff_lit);
    }
};
...
struct two_level_score{
    int hard_score;
    int soft_score;
    int score;
};

struct two_level_triple{
    int var_idx;
    int score;
...
struct lit{
    ...
    __int128_t                     mid_delta=INT32_MIN;
    bool                        is_equal=false;//true means a-b-k==0, else a-b-k<=0
    bool                        is_lia_lit=false;
    bool operator ==(const lit &rhs)
    {
        return( key==rhs.key&&pos_coff_var_idx==rhs.pos_coff_var_idx&&pos_coff==rhs.pos_coff&&neg_coff_var_idx==rhs.neg_coff_var_idx&&neg_coff==rhs.neg_coff);
    }
};
...
struct variable{
    ...
    std::vector<int>            soft_literals;
    std::vector<int>            soft_literal_clause;
    std::vector<__int128_t>     soft_literal_coff;
    std::vector<uint64_t>       soft_clause_idxs;
...
public:
    ...
    int                             local_soft;
    int                             N=0;
    std::list<int>                  pulled_arm;
    std::vector<double>              soft_score;
    std::vector<double>              soft_times;
    uint64_t                    soft_smooth_probability;
    std::vector<int>            soft_threshold;
    std::vector<int>            soft_inc;
    int                         best_step;
    int                         reduced_bound;
    int                         init_bound;
    int                         issat=0;
    ...
    int                         is_weighted;
    int                         h_inc;
    std::vector<int>            soft_clause_lit;//soft new 
    std::vector<soft_clause>         _soft_clauses;//soft new 
    uint64_t                    _num_soft_clauses;//soft new
    std::vector<int>            soft_clause_weight;//soft new
    int                         best_found_obj;//soft new
    uint64_t                    _num_vars;
    ...
    int                         _lit_in_unsat_soft_clause_num;//soft new
    int                        _bool_lit_in_unsat_soft_clause_num;//soft new
    //input transformation
    int                        untabu_pick(__int128_t &best_value);
    int                        pick_rand_soft_clause();
    void                       update_soft_clause_weight();
    void                       smooth_soft_clause_weight();
    int                        pick_bool_lia_var();
    two_level_triple            two_level_find_best_tabu();
    void                        restart();
    two_level_score             critical_score_two_level(uint64_t var_idx, __int128_t change_value);
    int                         pick_critical_move_dist(__int128_t &best_value);
    void                        initial_bound(std::string &instring);
    ...
    uint64_t                    transfer_name_to_tmp_var1(std::string &name,int flag);
    void                        reduce_vars();//reduce the x-y in all lits to new var z
    void                        build_softlits(std::vector<std::string >lit_arr,int lit_num);
    void                        build_soft_weight(std::vector<std::string>lit_weight);
    ...
    int                         pick_critical_two_level();
    int                         pick_critical_pair();
    int                         pick_critical_two_level_bool();
    two_level_triple            two_level_find_best();
    int                         two_level_find_best_bool();
    int                         pick_critical_move(__int128_t &best_value);
    int                         pick_critical_move_bool();
    int                         is_in_bound(int var_idx, __int128_t change_value);
    void                        critical_move(uint64_t var_idx,__int128_t change_value);
    void                        critical_move_pse(uint64_t var_idx,__int128_t change_value);
    ...
    void                        insert_operation_pair(int var_idx,__int128_t change_value,int &operation_idx,int l_idx);
    ...
    int                         critical_score_pair(uint64_t var_idx_1,__int128_t change_value_1,uint64_t var_idx_2,__int128_t change_value_2);
