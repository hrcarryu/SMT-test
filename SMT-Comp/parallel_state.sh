...
mkdir -p "$path_directory/isRight"
# 遍历所有组文件，并使用run.sh脚本处理每一组
for group_file in "${path_directory}/group_"*; do
    group_name=$(basename "$group_file")
    ./run_state.sh "$group_file" "$output_directory" "$input_directory" "$path_directory/isRight" > "${path_directory}/isRight/${group_name}.txt" 2>&1 &
done
